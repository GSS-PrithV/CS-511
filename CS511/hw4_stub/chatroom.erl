...
do_register(State, Ref, ClientPID, ClientNick) ->
	NewRegisters = maps:put(ClientPID,ClientNick,State#chat_st.registrations),
	ClientPID!{ self(), Ref, connect, State#chat_st.history},
	NewState = State#chat_st{registrations = NewRegisters}.

%% This function should unregister a client from this chatroom
do_unregister(State, ClientPID) ->
	Regs = maps:remove(ClientPID, State#chat_st.registrations),
    State#chat_st{ name = State#chat_st.name, registrations = Regs, history = State#chat_st.history}.

%% This function should update the nickname of specified client.
do_update_nick(State, ClientPID, NewNick) ->
	%% updpates the nickname lol
    State#chat_st{ name = State#chat_st.name, registrations = maps:update(ClientPID, NewNick, State#chat_st.registrations), history = State#chat_st.history}.

%% This function should update all clients in chatroom with new message
%% (read assignment specs for details)
do_propegate_message(State, Ref, ClientPID, Message) ->
    ClientPID!{self(), Ref, ack_msg},
	%% gets the person sendong and removes them from the list of people getting
	Sender = maps:get(ClientPID, State#chat_st.registrations),
	Recip = maps:remove(ClientPID,State#chat_st.registrations),
	%% send mesage to all people in Recip list
	lists:foreach(fun (Clint) -> Clint!{request, self(), Ref, {incoming_msg, Sender, State#chat_st.name, Message}} end,maps:keys(Recip)),
	NewState = State#chat_st{history = State#chat_st.history ++ [{Sender, Message}]}.
