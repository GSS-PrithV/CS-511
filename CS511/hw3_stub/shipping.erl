...
get_ship(Shipping_State, Ship_ID) ->
    Ship = Shipping_State#shipping_state.ships, lists:keyfind(Ship_ID,#ship.id,Ship).

get_container(Shipping_State, Container_ID) ->
    Container = Shipping_State#shipping_state.containers, lists:keyfind(Container_ID,#container.id,Container).

get_port(Shipping_State, Port_ID) ->
    Port = Shipping_State#shipping_state.ports, lists:keyfind(Port_ID,#port.id,Port).

get_occupied_docks(Shipping_State, Port_ID) ->
    Dokc = Shipping_State#shipping_state.ship_locations,
    ListDock = lists:filter(fun ({P, _D, _S}) -> P =:= Port_ID end,Dokc),
    lists:map(fun({_P, D, _S}) -> D end, ListDock).

get_ship_location(Shipping_State, Ship_ID) ->
    Dock = Shipping_State#shipping_state.ship_locations,
    case lists:keyfind(Ship_ID, 3, Dock) of
...
get_container_weight(Shipping_State, Container_IDs) ->
    case Container_IDs of
        [] -> 0;
        [_H | _T] ->lists:foldr(fun(ID, Weight) -> (get_container(Shipping_State, ID))#container.weight + Weight end, 0, Container_IDs);
        _E -> error
    end.
get_ship_weight(Shipping_State, Ship_ID) ->
    Cargo =  Shipping_State#shipping_state.ship_inventory,
    case maps:find(Ship_ID,Cargo) of
        {ok,Car} -> get_container_weight(Shipping_State,Car);
        _E -> error
    end.

load_ship(Shipping_State, Ship_ID, Container_IDs) ->
    {Port, _Dock} = get_ship_location(Shipping_State,Ship_ID),
    MaxWeight = (get_ship(Shipping_State,Ship_ID))#ship.container_cap,
    case Container_IDs of
        [] -> Shipping_State;
        [_H | _T] -> 
            {ok, SInventory} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
            {ok, PInventory} = maps:find(Port, Shipping_State#shipping_state.port_inventory),
            case ((length(Container_IDs) + length(SInventory)) =< MaxWeight) and is_sublist(PInventory, Container_IDs) of
                true ->  
                    NSI = lists:merge(SInventory, Container_IDs),                %league of legends reference (im sorry)
                    PSI = lists:filter(fun(ID) -> not lists:member(ID, Container_IDs) end, PInventory),
                    #shipping_state{ships=Shipping_State#shipping_state.ships,containers=Shipping_State#shipping_state.containers, 
                    ports=Shipping_State#shipping_state.ports,ship_locations=Shipping_State#shipping_state.ship_locations,
                    ship_inventory=maps:put(Ship_ID, NSI, Shipping_State#shipping_state.ship_inventory),
                    port_inventory=maps:put(Container_IDs, PSI, Shipping_State#shipping_state.port_inventory)};
                false -> error
            end;
        _E -> error
    end.
...
unload_ship_all(Shipping_State, Ship_ID) ->
    {ok,SInventory} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
    unload_ship(Shipping_State, Ship_ID, SInventory).

unload_ship(Shipping_State, Ship_ID, Container_IDs) ->
    {Port, _Dock} = get_ship_location(Shipping_State,Ship_ID),
    MaxWeight = (get_port(Shipping_State,Port))#port.container_cap,
    {ok, SInventory} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
    {ok, PInventory} = maps:find(Port, Shipping_State#shipping_state.port_inventory),
    case ((length(Container_IDs) + length(PInventory)) =< MaxWeight) and is_sublist(SInventory, Container_IDs) of
        true ->  
            NSI = lists:filter(fun(ID) -> not lists:member(ID, Container_IDs) end, SInventory), %league of legends esports team reference AGAIIIIN! (im sorry)
            PSI = lists:merge(PInventory, Container_IDs),       
            #shipping_state{ships=Shipping_State#shipping_state.ships,containers=Shipping_State#shipping_state.containers, 
            ports=Shipping_State#shipping_state.ports,ship_locations=Shipping_State#shipping_state.ship_locations,
            ship_inventory=maps:put(Ship_ID, NSI, Shipping_State#shipping_state.ship_inventory),
            port_inventory=maps:put(Port, PSI, Shipping_State#shipping_state.port_inventory)};
        false -> error
    end.    
