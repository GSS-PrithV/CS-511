	...
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		false ->
			...
			receive 
				...
				Connect1 = maps:put(ChatName, ChatPID, State#cl_st.con_ch),
				State1 = State#cl_st{con_ch = Connect1},
                {History, State1}
			end;
		true -> {err, State}
	end.
...
do_leave(State, Ref, ChatName) ->
	Balls = State#cl_st.con_ch,
    case maps:is_key(ChatName, State#cl_st.con_ch) of
		false -> {err, State};
		true -> whereis(server)!{self(), Ref, leave, ChatName},
		receive
			{LeaveChat, Ref, ack_leave} ->
			NewState = State#cl_st{con_ch = maps:remove(ChatName, Balls)},
			{ok, NewState}
		end
	end.
...
do_quit(State, Ref) ->
	...
	receive {Dunno, Ref, ack_quit} -> {ack_quit, State}
	end,
	exit(normal).
